...
export default function ConnexionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push("/mon-compte"); router.refresh();
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Gérez vos annonces</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input required type={showPwd ? "text" : "password"} value={form.password} onChange={(e) => set("password", e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            Pas de compte ? <Link href="/inscription" className="text-blue-700 font-medium hover:underline">Créer mon compte</Link>
          </p>
        </div>
      </div>
    </div>
